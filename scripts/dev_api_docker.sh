...
"${COMPOSE[@]}" exec api sh -lc "mix local.hex --force && mix local.rebar --force && mix deps.get && mix ecto.setup"

echo "Starting Phoenix API in container on http://localhost:4000 ..."
exec "${COMPOSE[@]}" exec api sh -lc "PORT=4000 PHX_DOCKER=1 mix phx.server"
